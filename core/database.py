...
db_path = conf.get('Path', 'database')
...
class DataBase:
    ...
    def add_data(self, data: list) -> None:
        """
        Принимаем на вход список кортежей [ (ip, device_name, vendor, auth_group, backup_group) ]
        """
        for row in data:
            try:
    ...
    def show_table(self):
        ...
        print(
            tabulate(
                ...
                headers=['ip', 'device_name', 'vendor', 'auth_group', 'backup_group'],
                tablefmt="presto",
                showindex="always"
            )
        )
